...
/**
 * processCmds_withLogOp - handles the processing of commands
 * separated by logical && or || operators
...
int processCmds_withLogOp(char *cmdline, char **argv, pdir_t **dirHead,
		char *envp[], alias **aliasHead, char *pathValcpy)
{
	...
	{
		j = 0;
		while (cmdcpy[i + j] != '&' && cmdcpy[i + j] != '|' && cmdcpy[i + j] != '\0')
			++j;
